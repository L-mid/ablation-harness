...
from ..configs import LoggingCfg
...
class Logger(Protocol):
    """Logging Template abstract. (all inclusive, includes should_log)"""

    def on_run_start(self, cfg: Any) -> None: ...
    def on_epoch_start(self, epoch: int) -> None: ...
    def should_log(self, step: int) -> bool: ...  # <- bool, not None
...
# 1) Backend sink protocol (no should_log)
class Sink(Protocol):
    """Logging Template abstract. (for actual loggers)"""
...
class WandbLogger:
    ...
    def on_run_start(self, cfg: Any) -> None:
        ...
        cfg_dict = _as_dict(cfg)
        self._run = self._wandb.init(**{k: v for k, v in self._init_kwargs.items() if v is not None})  # none resoluton problem
...
class MultiLogger:
    ...
    def __init__(self, sinks: Iterable[Sink], every: int = 1) -> None:
        """
...
# ---- Factory ---------------------------------------------------------------
def build_logger(logging_cfg: LoggingCfg | dict, run_dir: str | Path, run_name: Optional[str] = None) -> Logger:
    """
    ...
    lc = _as_dict(logging_cfg)  # works for dataclass or dict
    if not lc or not lc.get("enable", True):
        return NullLogger()

    run_dir = Path(_ensure_dir(str(run_dir)))
    backends = lc.get("backends", ["tensorboard"])
    every = int(lc.get("log_every_n_steps", 1))  # <- cadence lives in LoggingCfg

    loggers: List[Sink] = []

    if "tensorboard" in backends:
        tb_dir = run_dir / "logs" / "tb"
        loggers.append(TBLogger(log_dir=str(tb_dir), flush_secs=int(lc.get("tensorboard", {}).get("flush_secs", 10))))

    if "wandb" in backends:
        wb = lc.get("wandb", {})
        rn = run_name or wb.get("run_name") or "run"
        loggers.append(
            WandbLogger(
    ...
    if not loggers:
        return NullLogger()
    return MultiLogger(loggers, every=every)  # <- pass cadence here
